...
const UserMedia = ({ url, muted, playing, className, myVideo, username, userProfile, speakerMuted = false, hideElements = false }: UserMediaProps) => {
                ...
                <>
                    ...
                    {!playing && (
                        <div className="select-none w-full h-full bg-call-primary/50 flex items-center justify-center">
                            <UserAvatar username={username} userProfile={userProfile} size="large" />
                        </div>
                    )}
                </>
            ) : playing ? (
